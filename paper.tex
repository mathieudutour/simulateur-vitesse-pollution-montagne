...
burned during the first $T_{\text{cs}}$ seconds, per Weilenmann
\textit{et al.}~\cite{weilenmann2009}. The default trip is treated
as a single continuous outing (the engine remains warm during the
brief stop at the medical centre); a user wanting to model two
separate cold starts can double the slider value.
